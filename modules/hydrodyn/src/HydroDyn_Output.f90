...
!----------------------------------------------------------------------------------------------------
   ...
   if (allocated(InputFileData%OutList)) then
      ...
      IF ( ErrStat >= AbortErrLev ) RETURN
         ...
         IF (ALLOCATED( p%Morison%OutParam ) .AND. size(p%Morison%OutParam) > 0) THEN
            hasMorisonOuts = .TRUE.
...
!----------------------------------------------------------------------------------------------------
   ...
   IF ( (ALLOCATED( p%OutParam         ) .AND. p%NumOuts > 0         ) .OR. &
        (ALLOCATED( p%Morison%OutParam ) .AND. size(p%Morison%OutParam) > 0 ) ) THEN ! Output has been requested so let's open an output file
